        ...
        with urllib.request.urlopen(req, context=ctx, timeout=8) as response:
            ...
            if data.get("estabelecimento"):
                ...
                return {
                    ...
                    "email": email,
                    "telefone": tel,
                    "municipio": est.get("municipio", {}).get("descricao", ""),
                    "uf": est.get("estado", {}).get("sigla", "")
                }
    except Exception as e:
        print(f"  ⚠️ CNPJ {cnpj} não encontrado: {e}")
    return None
...
def montar_html(result):
    ...
    <div style="background:#f9f9f9;border-radius:8px;padding:14px 16px;margin-bottom:20px;text-align:center;">
        <table style="width:100%;border-collapse:collapse;">
            <tr>
                <td style="padding:8px;"><div style="font-size:24px;font-weight:bold;color:#333;">{total}</div><div style="font-size:11px;color:#888;text-transform:uppercase;">Total</div></td>
                <td style="padding:8px;"><div style="font-size:24px;font-weight:bold;color:#ff4d00;">{incendio}</div><div style="font-size:11px;color:#888;text-transform:uppercase;">Incêndio</div></td>
                <td style="padding:8px;"><div style="font-size:24px;font-weight:bold;color:#1976d2;">{termico}</div><div style="font-size:11px;color:#888;text-transform:uppercase;">Térmico</div></td>
                <td style="padding:8px;"><div style="font-size:24px;font-weight:bold;color:#e53935;">{alta}</div><div style="font-size:11px;color:#888;text-transform:uppercase;">Urgente</div></td>
            </tr>
        </table>
    </div>
